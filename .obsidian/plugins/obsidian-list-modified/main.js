...
var ListModified = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.onCacheChange = serialize((file, _data, cache) => __async(this, null, function* () {
      const modifiedFile = file;
      let dailyNote;
      try {
        dailyNote = (0, import_obsidian_daily_notes_interface.getDailyNote)((0, import_obsidian2.moment)(), (0, import_obsidian_daily_notes_interface.getAllDailyNotes)());
      } catch (e) {
        console.log("unable to create daily note!: " + e.message);
      }
      if (!dailyNote) {
        new import_obsidian2.Notice(`Creating Daily Note since it did not exist...`);
        dailyNote = yield (0, import_obsidian_daily_notes_interface.createDailyNote)((0, import_obsidian2.moment)());
      }
      if (modifiedFile === dailyNote)
        return;
      if (this.fileIsLinked(modifiedFile.path, dailyNote.path))
        return;
      if (!this.fileMeetsTagRequirements(cache))
        return;
  ...
  appendLink(dailyNote, currentFile) {
    return __async(this, null, function* () {
      const content = yield this.app.vault.read(dailyNote);
      const outputFormat = this.settings.outputFormat;
      const resolvedOutput = outputFormat.replace("[[link]]", this.app.fileManager.generateMarkdownLink(currentFile, dailyNote.path));
      const newContent = content.slice(-1) === "\n" ? content + resolvedOutput + "\n" : content + "\n" + resolvedOutput;
      yield this.app.vault.modify(dailyNote, newContent);
    });
  }
  fileIsLinked(currentFilePath, dailyNotePath) {
    const links = Object.keys(this.app.metadataCache.resolvedLinks[dailyNotePath]);
    if (!links)
      return false;
    return links.some((l) => l === currentFilePath);
  }
  fileMeetsTagRequirements(fileCache) {
    const currentFileTags = (0, import_obsidian2.getAllTags)(fileCache);
    const ignoredTags = this.settings.tags.replace(/\s/g, "").split(",");
    return !ignoredTags.some((ignoredTag) => currentFileTags.contains(ignoredTag));
  }
  loadSettings() {
    return __async(this, null, function* () {
